...
Here we demonstrate building and running a single-file "barebones" [Soklet](https://www.soklet.com) application with nothing but the [soklet-3.3.0.jar](https://repo1.maven.org/maven2/com/soklet/soklet/3.3.0/soklet-3.3.0.jar) and the JDK.  There are no other libraries or frameworks, no Servlet container, no Maven build process - no special setup is required.
...
```shell
javac -parameters -processor com.soklet.SokletProcessor -cp soklet-3.3.0.jar -d build src/com/soklet/barebones/App.java
```

#### Run

```shell
java -cp soklet-3.3.0.jar:build com/soklet/barebones/App
```
...
RUN mkdir -p /app/src
COPY src /app/src
COPY soklet-3.3.0.jar /app

# Build the app
WORKDIR /app
RUN javac -parameters -processor com.soklet.SokletProcessor -cp soklet-3.3.0.jar -d build src/com/soklet/barebones/App.java

# Unprivileged user for runtime
USER 1000

CMD ["/bin/sh", "-c", "exec java -cp soklet-3.3.0.jar:build com/soklet/barebones/App"]
```
