...
Here we demonstrate building and running a single-file "barebones" [Soklet](https://www.soklet.com) application with nothing but the [soklet-2.0.2.jar](https://repo1.maven.org/maven2/com/soklet/soklet/2.0.2/soklet-2.0.2.jar) and the JDK.  There are no other libraries or frameworks, no Servlet container, no Maven build process - no special setup is required.
...
```shell
javac -parameters -processor com.soklet.SokletProcessor -cp soklet-2.0.2.jar -d build src/com/soklet/barebones/App.java 
```

#### Run

```shell
java -cp soklet-2.0.2.jar:build com/soklet/barebones/App
```
